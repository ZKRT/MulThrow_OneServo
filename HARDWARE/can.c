...
//设置过滤器0，接收任何数据，只接受ID=1的数据
	...
	CAN_FilterInitStructure.CAN_FilterIdHigh=(DEVICE_TYPE_THROW<<5);////32位ID，这个保存ID[3:0]
...
//将子模块的数据返回给管理模块
uint8_t CAN1_send_message_fun(uint8_t *message, uint8_t len)//合成执行之前就该知道对哪个设备操作了
{
	//假设一共50个字
	uint8_t count;		              //发送的次数，一次发送8个字
	uint8_t time;
	
	time = len/8;             //time=6
	
	for (count = 0; count < time; count++)//这里发送6次，每次8个字节
	{
		Can_Send_Msg(message, 8);
		message += 8;
		delay_us(999);
	}
	if (len%8)                          //如果还有不足8个字节的数据，再调用一次CAN发送
	{
		Can_Send_Msg(message, len%8);//这里将剩余的2个字节再发送出去
		delay_us(999);
	}
